 ...
 * generated by rend_get_service_id). XXX: now not exactly  */
int
rend_valid_service_id(const char *query)
{
  if (!(strlen(query) == REND_SERVICE_ID_LEN_BASE32 ||
        strlen(query) == REND_AUTH_DATA_HYPHEN_SERVICE_ID_LEN_BASE32))
    return 0;
  char *dash = strrchr(query, '-');
  char *service_id = query;
  /* NUL-termintate for a moment */
  if (dash) {
    *dash = 0;
    /* Check auth length */
    if (strspn(query, BASE32_CHARS) != REND_DESC_COOKIE_LEN_BASE32) {
      /* Make a dash again */
      *dash = '-';
      return 0;
    }
  ...
  if (strspn(service_id, BASE32_CHARS) != REND_SERVICE_ID_LEN_BASE32)
    return 0;
